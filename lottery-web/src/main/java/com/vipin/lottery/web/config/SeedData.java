...
public class SeedData implements ApplicationRunner {
    ...
    public SeedData(HistoryStore store, HistoryParser parser) {
        this.store = store;
        this.parser = parser;
    }
    ...
    public void run(ApplicationArguments args) {
        if (!enabled) return;
        for (var entry :
                Map.of("PB", "files/pb/pb-sorted.txt", "MM", "files/archive/mm-sorted.txt")
                        .entrySet()) {
            ...
            try {
                store.save(
                        entry.getKey(),
                        parser.parse(Files.readString(path), entry.getKey()),
                        "Initial repository history",
                        false);
            } catch (Exception e) {
                LoggerFactory.getLogger(getClass())
                        .warn("Could not seed {}: {}", entry.getKey(), e.getMessage());
            }
        }
    }
}
