package com.vipin.lottery.core.source;
...
public class PowerballSource {
    ...
    public static final LocalDate START = LocalDate.of(2015, 10, 7);
    ...
    public List<DrawRecord> fetch() throws IOException, InterruptedException {
        ...
        try (InputStream input = response.body()) {
            if (response.statusCode() != 200)
                throw new IOException("Official source returned HTTP " + response.statusCode());
            byte[] bytes = input.readNBytes(3_000_001);
            if (bytes.length > 3_000_000)
                throw new IOException("Official history exceeds size limit");
            return parse(
    ...
    public List<DrawRecord> parse(String csv, LocalDate today) {
        TreeMap<LocalDate, DrawRecord> records = new TreeMap<>();
        for (String line : csv.split("\\R")) {
            if (line.isBlank()) continue;
            String[] f = line.split(",", -1);
            if ((f.length != 10 && f.length != 11) || !f[0].equals("Powerball"))
                throw new IllegalArgumentException("Unexpected official CSV format");
            LocalDate date =
                    LocalDate.of(
                            Integer.parseInt(f[3]), Integer.parseInt(f[1]), Integer.parseInt(f[2]));
            ...
            if (records.put(date, row) != null)
                throw new IllegalArgumentException("Duplicate official draw " + date);
        }
        ...
        if (records.lastKey().isBefore(expected))
            throw new IllegalArgumentException(
                    "Official source is not yet current. Latest: " + records.lastKey());
        for (LocalDate d = START; !d.isAfter(records.lastKey()); d = d.plusDays(1))
            if (scheduled(d) && !records.containsKey(d))
                throw new IllegalArgumentException("Gap in official history: " + d);
    ...
    static boolean scheduled(LocalDate date) {
        return date.getDayOfWeek() == DayOfWeek.WEDNESDAY
                ...
                || (date.getDayOfWeek() == DayOfWeek.MONDAY
                        && !date.isBefore(LocalDate.of(2021, 8, 23)));
    }
}
