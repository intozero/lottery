...
public final class Statistics {
    ...
    public Statistics(int maximum) {
        if (maximum < 5 || maximum > 99)
            throw new IllegalArgumentException("Maximum must be between 5 and 99");
        this.maximum = maximum;
        totals = new int[maximum + 1];
        last = new int[maximum + 1];
        minGap = new int[maximum + 1];
        maxGap = new int[maximum + 1];
        lastDates = new LocalDate[maximum + 1];
    }

    public void accept(DrawRecord draw) {
        if (latest != null && !draw.date().isAfter(latest))
            throw new IllegalArgumentException("Draw dates must be unique and increasing");
        List<Integer> balls = draw.whites();
        for (int ball : balls)
            if (ball > maximum) throw new IllegalArgumentException("White ball exceeds " + maximum);
        drawCount++;
        latest = draw.date();
        ...
        String pattern =
                Arrays.stream(buckets).mapToObj(Integer::toString).collect(Collectors.joining("-"));
        ...
        for (int i = 0; i < buckets.length; i++) {
            if (buckets[i] > 0)
                occupancies.merge(rangeLabel(i) + ": " + buckets[i] + " balls", 1, Integer::sum);
        }
    }
        ...
        for (int number = 1; number <= maximum; number++) {
            rows.add(
                    new NumberStats(
                            number,
                            totals[number],
                            drawCount - last[number],
                            totals[number] < 2 ? null : minGap[number],
                            totals[number] < 2 ? null : maxGap[number],
                            lastDates[number]));
        }
    ...
    public Map<String, Integer> patterns() {
        return Collections.unmodifiableMap(new TreeMap<>(patterns));
    }

    public Map<String, Integer> shapes() {
        return Collections.unmodifiableMap(new TreeMap<>(shapes));
    }

    public Map<String, Integer> occupancies() {
        return Collections.unmodifiableMap(new TreeMap<>(occupancies));
    }
    ...
    public String rangeLabel(int bucket) {
        return Math.max(1, bucket * 10) + "-" + Math.min(maximum, bucket * 10 + 9);
    }
}
