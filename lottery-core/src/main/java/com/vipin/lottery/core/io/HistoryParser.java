package com.vipin.lottery.core.io;
...
public class HistoryParser {
    ...
    public List<DrawRecord> parse(String text, String game) {
        ...
        if (text == null || text.length() > 3_000_000)
            throw new IllegalArgumentException("History must be at most 3 MB");
            ...
            try {
                ...
                if (fields.length != 6 && fields.length != 7)
                    throw new IllegalArgumentException(
                            "Expected date, five white balls, and optional special ball");
                LocalDate date = LocalDate.parse(fields[0], DATE);
                if (date.isAfter(LocalDate.now(ZoneId.of("America/New_York"))))
                ...
                DrawRecord row =
                        new DrawRecord(
                                date,
                                balls,
                                fields.length == 7 ? Integer.valueOf(fields[6]) : null);
                if (row.whites().get(4) > max)
                    throw new IllegalArgumentException("White ball exceeds " + max);
                DrawRecord old = records.putIfAbsent(date, row);
                if (old != null && !old.equals(row))
                    throw new IllegalArgumentException("Conflicting duplicate date " + date);
        ...
        if (records.size() > 30000)
            throw new IllegalArgumentException("At most 30,000 draws per import");
