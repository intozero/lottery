...
final class TextTable {
    ...
    TextTable(String[] headers, boolean... numeric) {
        if (headers.length != numeric.length)
            throw new IllegalArgumentException("Column alignment count mismatch");
    ...
    private void line(Writer out, String[] values, int[] widths, boolean alignNumbers)
            throws IOException {
        out.write("|");
        for (int i = 0; i < widths.length; i++) {
