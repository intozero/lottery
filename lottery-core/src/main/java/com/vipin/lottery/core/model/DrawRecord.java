package com.vipin.lottery.core.model;
...
public record DrawRecord(LocalDate date, List<Integer> whites, Integer special) {
    public DrawRecord {
        ...
        if (special != null && (special < 1 || special > 99))
            throw new IllegalArgumentException("Invalid special ball");
    }

    public int sum() {
        return whites.stream().mapToInt(Integer::intValue).sum();
    }
    ...
    public String values() {
        return String.join(" ", whites.stream().map(Object::toString).toList())
    ...
    public static int maximum(String game) {
        return switch (game) {
            case "PB" -> 69;
            case "MM" -> 75;
            default -> throw new IllegalArgumentException("Game must be PB or MM");
        };
    }
}
