...
import com.vipin.lottery.core.analysis.NumberStats;
import com.vipin.lottery.core.analysis.Statistics;
import com.vipin.lottery.core.model.DrawRecord;
import java.io.*;
import java.util.*;

/** Streams deterministic reports without retaining every historical snapshot. */
public final class CombinedReport {
    public enum Action {
        LAST,
        SIM,
        NUM_OCCUR,
    ...
    public void write(List<DrawRecord> input, int maximum, Action action, Writer output)
            throws IOException {
        ...
        new TextTable(new String[] {"Setting", "Value"}, false, false)
                ...
                .write(output);
        output.write(
                "Since: draws after last appearance; never seen = snapshot draw count.\n"
    ...
    private void snapshot(Statistics stats, Action action, Writer out) throws IOException {
        ...
        if (action == Action.RAN) {
            ...
            long occupied =
                    stats.occupancies().values().stream().mapToLong(Integer::longValue).sum();
        ...
        if (action == Action.LAST) {
            numbers.sort(
                    Comparator.comparingInt(NumberStats::getTotal)
                            .thenComparingInt(NumberStats::getNumber));
            numberTable("Numbers by total ascending", numbers, out);
            numbers.sort(
                    Comparator.comparingInt(NumberStats::getSince)
                            .reversed()
                            .thenComparingInt(NumberStats::getNumber));
        ...
        TextTable ranges =
                new TextTable(
                        new String[] {"Range", "Total appearances", "Sum of since"},
                        false,
                        true,
                        true);
        for (int bucket = 0; bucket <= stats.getMaximum() / 10; bucket++) {
            ...
            for (NumberStats number : numbers) {
                if (number.getNumber() / 10 == bucket) {
                    total += number.getTotal();
                    since += number.getSince();
                }
            }
    ...
    private void numberTable(String heading, List<NumberStats> numbers, Writer out)
            throws IOException {
        out.write(heading + "\n");
        TextTable table =
                new TextTable(
    ...
    private void frequencies(
            String heading, Map<String, Integer> values, long denominator, Writer out)
            throws IOException {
        out.write(heading + " (percent denominator: " + denominator + ")\n");
        TextTable table =
                new TextTable(new String[] {"Category", "Count", "Percent"}, false, true, true);
        ...
        for (Map.Entry<String, Integer> row : rows)
            table.row(
                    row.getKey(),
                    row.getValue(),
                    String.format(Locale.ROOT, "%.2f%%", 100.0 * row.getValue() / denominator));
        table.write(out);
    }
}
